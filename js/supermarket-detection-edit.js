...
(function (mui, $) {
    ...
    setData(registrant, 'fullName', '/batch/detection/getEmployeeSelect', null, {
    ...
    $(".product-detail").on("click", ".batch-tap", function () {
        ...
        setData(registrant, 'collectDate', '/batch/detection/getBatchTimeByProductId', {
            productId: row.id
        }, {
            success: function (data) {
                $.each(data, function (index, value) {
    ...
    function loadOnSaleProduct(data) {
        $.ajax({
            cache: false,
            type: 'get',
            url: Route.baseUrl +"/batch/detection/getProductsHasBatchByEntpId",
            data: data,
            success: function (data) {
                $(".product-list").empty();
                $.each(data, function (index, value) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data && data.result && data.result.resultCode == 9) {
                        ...
                        // window.location.href = "supermarket-detection.html";
                        var url = UrlParam.getParam('source')=="index"?"index.html":"supermarket-detection.html";
                        mui.openWindow({
                            url: url,
                            createNew: true,
                            styles: {
