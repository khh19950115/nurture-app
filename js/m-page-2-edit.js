(function (mui, $) {
    ...
    if (id) {
        ...
        if(copy){
            ...
            $.ajax({
                cache: false,
                type: 'get',
                url: Route.baseUrl +"/stock/outstock/getOutstockDetailList",
                data: {
                    pageSize: 1000,
                    pageNumber: 1,
                    outstockId: id
    ...
    setData(supplier, 'customerName', supplierUrl, null, {
    ...
    if(common.isOperating) {
        ...
        var operatingUrl = '/stock/outstock/wholeSaleMarket/getAgencyByMarketId';
        setData(operatinghouseholds, 'entpName', operatingUrl, null, {
            id: outstock.result.entpId,
            container: $("#agencyId"),
        ...
        if(!id){
            $("#operating-households").click(function () {
                ...
                operatinghouseholds.show(function (items) {
                    if(items[0].id){
                        if($("#agencyId").data("val")!=items[0].id){
    ...
    function loadOnSaleProduct(data) {
        $.ajax({
            ...
            type: 'get',
            data: data,
            url: Route.baseUrl +"/stock/outstock/getBatchedProductByAgencyId",
            success: function (data) {
                $(".product-list").empty();
                $.each(data, function (index, value) {
    ...
    var registrantUrl = '/stock/outstock/getEmployeeByEntpId';
    ...
    setData(registrant, 'fullName', registrantUrl, null, {
        id: outstock.result.employeeId || StorageItem.getEmployeeId(),
    ...
    $("#topRight").click(function () {
        ...
        try {
            ...
            $.ajax({
                type: 'post',
                url: Route.baseUrl + "/stock/outstock/agency/saveOutstock",
                contentType: "application/json;charset=UTF-8",
                dataType: 'json',
                ...
                success: function (data) {
                    if (data && data.result && data.result.resultCode == 9) {
                        ...
                        // window.location.href = "m-page-2.html";
                        var url = UrlParam.getParam('source')=="index"?"index.html":"m-page-2.html";
                        mui.openWindow({
                            url: url,
                            createNew: true,
                            styles: {
