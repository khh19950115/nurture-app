(function (mui, doc, $) {
    ...
    mui('body').on('tap', '.go-detail', function () {
        ...
        mui.openWindow({
            url: 'm-page-3-edit.html?id=' + row.id,
            createNew: true,
            styles: {
                cachemode:"noCache"
            }
    ...
    $("#resetBtn").on('click', function (event) {
        $("#agency input[type='checkbox']:checked,#detectionType input[type='checkbox']:checked,#isQualified input[type='checkbox']:checked").prop("checked", false);
...
function initOperatorData(){
    var settings = {
        cache: false,
        type: 'get',
        url: Route.baseUrl + "/batch/detection/getAgencyByEntpId",
        success: function (data) {
            $("#agency").empty();
