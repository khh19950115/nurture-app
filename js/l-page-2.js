...
var entpId = UrlParam.getParam('id');
(function(mui, doc,$) {
    mui.init({
        pullRefresh: {
	...
	mui('body').on('tap','.pro-add',function(){
        mui.openWindow({
            url: 'l-page-2-add.html?entpId='+entpId,
            createNew: true,
            styles: {
                cachemode:"noCache"
            }
	...
	mui('body').on('tap','.list-pro',function(){
        ...
        mui.openWindow({
            url: 'l-page-2-edit.html?id='+id+'&entpId='+entpId,
            createNew: true,
            styles: {
                cachemode:"noCache"
            }
    ...
    mui('body').on('tap' , '.screen-btn1' , function () {
        ...
        initOperatorList(initPageNumber ,content,"");
        fnClick();
    });
...
function initList(pageNumber , content , varietyIds) {
    ...
    //onSale=0 查询上架的产品
    var url = '/product/kind/getKindByEntpJrId?entpJrId='+entpId +'&pageSize=' + ComConst.PAGE_SIZE + '&pageNumber=' + pageNumber+ '&content=' + content+'&varietyIds[]='+varietyIds+'&onSale=0' + '&isApp=true';
    var ajaxSetting = {
        type: "GET",
        dataType: 'json',
        ...
        success: function (result) {
            ...
            $.each(list, function (i, rowData) {
                var html = '<li class="mui-table-view-cell">' +
                    ...
                    '<span>品类：</span><span>' + rowData.varietyName + '</span>' +
    ...
    $.ajax({
        ...
        success: function (data) {
            if (!data) {
                ...
                $.ajax({
                    type: 'POST',
                    url: Route.baseUrl +"/product/kind/deleteProduct",
                    data: {'productId': operatorId},
                    dataType: 'json',
                    success: function (data) {
                        ajaxlock = false;
