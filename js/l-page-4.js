...
(function(mui, doc,$) {
	...
	//编辑客户-企业
	mui('body').on('tap','.list-company',function(){
        var id = jQuery(this).find('.man-list-top2').data('id');
        var type= jQuery(this).find('.badge-left').text();
        if(type == "个人"){
            mui.openWindow({
                url: 'l-page-4-personal-edit.html?id=' + id,
                createNew: true,
                styles: {
                    cachemode:"noCache"
                }
            ...
            mui.openWindow({
                url: 'l-page-4-company-edit.html?id='+id,
                createNew: true,
                styles: {
                    cachemode:"noCache"
                }
	...
	$('.management-list').on('tap', '.mui-btn', function(event) {
		...
		mui.confirm('确认删除？', '删除确认', CONFIRM_BTN_LIST, function(e) {
			if (e.index == CONFIRM_BTN_YES) {
               ...
               var type= jQuery(li).find('.badge-left').text();
               if(type=="个人"){
                   deleteCustomer(id, li);
               }else{
                   deleteEntpCustomer(id, li);
               }
			} else {
				setTimeout(function() {
					$.swipeoutClose(li);
...
function initList(pageNumber , searchParam) {
    ...
    var url = '/entp/entpCustomer/searchCustomer?pageSize=' + ComConst.PAGE_SIZE + '&pageNumber=' + pageNumber + '&searchParam=' + searchParam;
    var ajaxSetting = {
        type: "GET",
        dataType: 'json',
        ...
        success: function (result) {
            ...
            if(result.pages == 0 || result.pages == 0){
                //mui.toast("没找到匹配的信息！");
                $('.tips').show();
            ...
            $.each(list, function (i, rowData) {
                var html = '<li class="mui-table-view-cell">' +
                    ...
                    '</div>' +
                    '<div class="mui-slider-handle list-company">' +
                    '<div class="man-list-top2 " data-id="' + rowData.id + '">' +
					'<span class="badge-left">' + rowData.systemReserve3 + '</span>'+
 ...
 * 删除个人客户
 */
function deleteCustomer(operatorId, li) {
    if (ajaxlock) {
        return;
    }
    ajaxlock = true;
    var url = '/entp/customer/delete';
    var ajaxSetting = {
        url: Route.baseUrl +url,
        type: "POST",
        dataType: 'json',
 ...
 * 删除企业客户
 */
function deleteEntpCustomer(operatorId, li) {
    if (ajaxlock) {
        return;
    }
    ajaxlock = true;
    var url = '/entp/entpCustomer/delete';
    var ajaxSetting = {
        url:Route.baseUrl + url,
        type: "POST",
        dataType: 'json',
